...
export type AccountHandlers = {
  ...
  'enablebanking-create-web-token': typeof enableBankingCreateWebToken;
  'enablebanking-poll-web-token': typeof enableBankingPollWebToken;
  'enablebanking-poll-web-token-stop': typeof enableBankingStopPolling;
...
async function linkEnableBankingAccount({
  ...
  // bank_id = "sessionId:accountUid" so sync can split them back out
  const bankId = `${sessionId}:${externalAccount.uid}`;
  const institution = { name: externalAccount.institution ?? t('Unknown') };
  const bank = await link.findOrCreateBank(institution, bankId);
...
async function enableBankingCreateWebToken({
  aspspName,
  aspspCountry,
}: {
  aspspName?: string;
  ...
  try {
    return await post(
      serverConfig.ENABLEBANKING_SERVER + '/create-web-token',
...
async function enableBankingPollWebToken({ state }: { state: string }) {
  ebStopPolling = false;
  const startTime = Date.now();
  const userToken = await asyncStorage.getItem('user-token');
...
async function enableBankingStopPolling() {
  ebStopPolling = true;
  return 'ok';
}
...
app.method('enablebanking-create-web-token', enableBankingCreateWebToken);
app.method('enablebanking-poll-web-token', enableBankingPollWebToken);
app.method('enablebanking-poll-web-token-stop', enableBankingStopPolling);
