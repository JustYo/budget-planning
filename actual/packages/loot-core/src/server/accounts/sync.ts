...
async function downloadEnableBankingTransactions(
  ...
  // bankId = "sessionId:accountUid" stored when the account was linked
  const [sessionId, accountUid] = bankId.split(':');

  const res = await post(
    getServer()!.ENABLEBANKING_SERVER + '/transactions',
    { sessionId, accountUid, startDate: since },
    { 'X-ACTUAL-TOKEN': userToken },
    60000,
  );
  ...
  logger.log('Enable Banking response:', res);
...
export async function syncAccount(
  ...
  if (acctRow.account_sync_source === 'simpleFin') {
    ...
    download = await downloadEnableBankingTransactions(bankId, syncStartDate);
