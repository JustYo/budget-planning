...
# Build the email notifier dependencies separately (has its own package.json)
...
# Email notifier
COPY --from=email-notifier-deps /notifier/node_modules /app/email-notifier/node_modules
COPY packages/sync-server/email-notifier/package.json /app/email-notifier/
COPY packages/sync-server/email-notifier/src /app/email-notifier/src

# Startup script
COPY packages/sync-server/start.sh /app/start.sh
