  ...
  return (
    ...
    switch (action.type) {
      case 'set-schedule': {
        ...
        // Extract payee_name text from conditions (preferred), fall back to _payee UUID
        const payeeCond = conds.payee;
        const payeeValue =
          payeeCond?.field === 'payee_name'
            ? (payeeCond.value as string)
            : (schedule._payee ?? null);

        const dateTolerance =
          (conds.date?.options as { tolerance?: number } | undefined)
            ?.tolerance ?? 2;
