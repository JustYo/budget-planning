...
export function BankSync() {
  ...
  const onSync = useCallback(
    async (account: AccountEntity) => {
      ...
      try {
        const res = await send('accounts-bank-sync', { ids: [account.id] });
        const errors: Array<{ code?: string; message?: string }> =
          res.errors ?? [];
        const loginRequired = errors.some(
          e => e.code === 'ITEM_LOGIN_REQUIRED',
        );

        if (loginRequired) {
          dispatch(
            pushModal({
              modal: { name: 'enablebanking-external-msg', options: {} },
            }),
          );
        } else {
          errors.forEach(e => {
        ...
        dispatch(
          addNotification({
            notification: {
              type: 'error',
              message: t('Sync failed. Please try again.'),
            },
          }),
        );
      } finally {
