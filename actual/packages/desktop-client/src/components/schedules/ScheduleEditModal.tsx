...
export function ScheduleEditModal({ id, transaction }: ScheduleEditModalProps) {
  ...
  const initialSchedule = useMemo(() => {
    ...
    const transFields = fromTrans
      ? (() => {
          const transPayeeName =
            transaction.payee && payees?.[transaction.payee]
              ? payees[transaction.payee].name
              : '';
          return {
                ...
                ? [
                    {
                      ...
                      field: 'payee_name' as const,
                      value: transPayeeName,
                    },
                  ]
                : []),
            ],
  ...
  const payeeName = state.fields.payee ?? null;
      ...
      {({ state: modalState }) => (
        <>
          <ModalHeader
            title={
              payeeName
                ? t(`Schedule: {{name}}`, { name: payeeName })
                : t('Schedule')
            }
            rightContent={
